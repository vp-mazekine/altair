...
object EVER {
    ...
    suspend fun getTokensInfo(
        ...
        val path = "$apiPrefix/tokens/address/$address"
        val (signature, nonce) = sign(path) ?: return null

        return try {
            tonApiClient.get<AccountTokenBalanceOutput>(apiEndpoint + path) {
                headers {
                    append("Accept", "application/json")
                    append("Accept-Encoding", "gzip, deflate, br")
                    append("Connection", "keep-alive")
    ...
    suspend fun getTokenInfo(
        ...
        val tokensInfo = getTokensInfo(address) ?: throw InternalError("API hasn't returned any data")

        return tokensInfo.firstOrNull { it.rootAddress == rootAddress }
    }

    suspend fun createTokenTransaction(
        fromAddress: String,
        toAddress: String,
        rootAddress: String,
        value: String = "",
        ...
        val path = "$apiPrefix/tokens/transactions/create"

        value.toBigDecimalOrNull()?.let {
            if (it < BigDecimal(0)) {
                logger.error("[createTokenTransaction] Transaction value must be positive")
                onFail?.let { it(TransactionFailReason.TX_VALUE_NOT_POSITIVE) }
                return null
            }
        } ?: run {
            logger.error("[createTokenTransaction] Transaction value cannot be empty")
            onFail?.let { it(TransactionFailReason.TX_VALUE_EMPTY) }
            return null
        }

        val body = SendTokenTransactionInput(
            fromAddress,
            toAddress,
            rootAddress,
            value,
        ...
        val response = try {
            tonApiClient.post<SendTokenTransactionOutput>(apiEndpoint + path) {
                headers {
                    append("Accept", "application/json")
                    append("Accept-Encoding", "gzip, deflate, br")
                    append("Connection", "keep-alive")
            ...
            logger.error(
                "[createTokenTransaction] Error ${e.response.status.value}: ${e.response.status.description} when trying to send transaction of $value tokens from address $fromAddress to $toAddress\n" +
                        e.response.readText()
            )
            onFail?.let { it(
                when {
            ...
            logger.error(
                "[createTokenTransaction] Error when trying to send transaction of $value tokens from address $fromAddress to $toAddress\n" +
                        e.message + "\n" +
                        e.stackTrace.joinToString("\n")
            )
            onFail?.let { it(
        ...
        if (response.data == null) {
            logger.error(
                "[createTokenTransaction] Error while creating the transaction of $value EVER from address $fromAddress to $toAddress\nError message: ${response.errorMessage}"
            )
            onFail?.let { it(
                when {
                    response.errorMessage!!.contains("Invalid value") -> TransactionFailReason.TX_VALUE_EMPTY
    ...
    suspend fun getTokenTransaction(
        txId: String? = null,
        messageHash: String? = null
    ): TokenTransactionOutputData? {
        if (txId == null && messageHash == null) return null
        ...
        val path = "$apiPrefix/tokens/transactions/$apiMethod/$searchValue"
        val (signature, nonce) = sign(path) ?: return null

        return try {
            tonApiClient.get<SendTokenTransactionOutput>(apiEndpoint + path) {
                headers {
                    append("Accept", "application/json")
                    append("Accept-Encoding", "gzip, deflate, br")
                    append("Connection", "keep-alive")
            ...
            logger.warn("Error getting transaction status: Status ${e.response.status}, Message: ${e.message}")
            null
        } catch (e: Exception) {
            logger.warn("Error getting transaction status: ${e.message}")
            null
        }
    }
